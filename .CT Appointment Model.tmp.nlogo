...
to init-globals
  set ticks-to-lift 60 ; base number of crane lifting procedure 15
  set ticks-to-rehandle 100 ; number of ticks it takes for crane to move container from one place to another in the same stack. 40
  set ticks-to-deliver 200 ;number of ticks it takes for crane to move container from stack to truck 50
  set ticks-to-move 10 ;number of ticks it takes for the crane to move to an adjacent container 6
...
; ad = administrative time (customs etc)

to-report avg-both-ad
  let x num-trucks-serviced
  if x = 0 [report 0]
  report total-admin-time / x
end
...
to deliver-container [the-container]
  ...
  ifelse ([z-cor] of the-container = pile-height) [ ;the-container is at the top
    ...
    ask the-truck [
      ...
      ;add total-admin-time
      set total-admin-time total-admin-time + my-admin-time - my-start-time
